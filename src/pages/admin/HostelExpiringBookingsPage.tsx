...
export default function HostelExpiringBookingsPage() {
  ...
  const filtered = useMemo(() => {
    let result = bookings.filter((b) => {
      ...
      return (
        ...
        (hostel?.name || '').toLowerCase().includes(s)
      );
    });
    result.sort((a, b) => {
  ...
  const handleExport = () => {
    const headers = ['S.No', 'Booking ID', 'Customer', 'Email', 'Phone', 'Hostel', 'Room', 'Bed', 'Start Date', 'End Date', 'Days Left'];
    const rows = filtered.map((b, i) => {
      const p = b.profiles as any; const h = b.hostels as any; const r = b.hostel_rooms as any; const bed = b.hostel_beds as any;
      return [i + 1, b.serial_number || b.id.substring(0, 8), p?.name || '', p?.email || '', p?.phone || '', h?.name || '', r?.room_number || '', bed?.bed_number || '', format(new Date(b.start_date), 'dd/MM/yyyy'), format(new Date(b.end_date), 'dd/MM/yyyy'), getDaysRemaining(b.end_date)];
    });
    const csv = [headers, ...rows].map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `hostel-expiring-bookings-${daysThreshold}d.csv`; a.click();
  };
      ...
      <Card className="border-border/50 shadow-sm">
        <CardHeader className="py-3 px-4 bg-muted/30 border-b">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-semibold text-foreground/90">Expiring Hostel Bookings</CardTitle>
        ...
        <CardContent className="p-0">
          ...
          {loading ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/30">
                      ...
                      <TableHead className="text-[11px] font-medium uppercase tracking-wider">Hostel / Room / Bed</TableHead>
                    ...
                    {paginated.map((booking, idx) => {
                      ...
                      return (
                        <TableRow key={booking.id} className="text-xs">
                          ...
                          <TableCell>
                            <div className="text-xs">{hostel?.name || 'N/A'}</div>
                            <div className="text-[10px] text-muted-foreground">
                              Room {room?.room_number || 'N/A'} · Bed {bed?.bed_number || 'N/A'}
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => navigate(`${routePrefix}/bookings/${booking.id}/hostel`)}>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground text-sm">
              ...
              No expiring hostel bookings found
            </div>
          )}
        </CardContent>
      </Card>
