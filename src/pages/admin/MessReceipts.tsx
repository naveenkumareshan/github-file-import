...
export default function MessReceipts() {
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      const messIds = [...new Set((data || []).map((r: any) => r.mess_id))];

      const [profilesRes, messesRes] = await Promise.all([
        userIds.length > 0 ? supabase.from('profiles').select('id, name, phone, email').in('id', userIds) : { data: [] },
        messIds.length > 0 ? supabase.from('mess_partners' as any).select('id, name').in('id', messIds) : { data: [] },
      ]);

      const profileMap = Object.fromEntries((profilesRes.data || []).map((p: any) => [p.id, p]));
      const messMap = Object.fromEntries(((messesRes.data || []) as any[]).map((m: any) => [m.id, m]));
      ...
      const mapped = (data || []).map((r: any) => ({
        ...
        messName: messMap[r.mess_id]?.name || '',
      }));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Mess Receipts</h1>
      ...
      <div className="border rounded-md">
        <Table>
          <TableHeader>
            <TableRow>
              {['S.No.', 'Receipt #', 'Student', 'Mess', 'Amount', 'Method', 'Txn ID', 'Date'].map(h => (
